"""
Weibull AFT nested 5x5-fold cross-validation
"""
...
class Objective(object):
    ...
    def __call__(self, trial) -> float:
        ...
        hparams = {
            ...
            "l1_ratio" : trial.suggest_loguniform("l1_ratio",1e-6, 1e-3),
            }              
    ...
    for i, outer in enumerate(cv_outer):
        ...
        with mlflow.start_run(run_name = run_name_nstd, nested=True) as run_nstd: 
            ...
            try:
                ...
                if MODUS == "experiment":
                    model.fit(X_dev_oh_y, duration_col="time",event_col="status")
                elif MODUS == "dev":
                    model.fit(X_dev_oh_y.iloc[:100,-50:], duration_col="time",event_col="status")
            except:
                print("\n trial failed")
                # https://optuna.readthedocs.io/en/stable/faq.html#how-are-nans-returned-by-trials-handled
                ...
                # evaluate model
                dev_charrell = model.score(X_dev_oh_y, scoring_method="concordance_index")
                test_charrell = model.score(X_test_oh_y, scoring_method="concordance_index")
